...
public class ReflectUtil {
    ...
    public static Field getField(Class<?> sourceClass, String fieldName, boolean isFindDeclaredField, boolean isUpwardFind){
        ...
        try {
            ...
            if(isUpwardFind){
                Class<?> clazz = sourceClass.getSuperclass();
    ...
    public static List<Field> getFields(Class<?> sourceClass, boolean isGetDeclaredField, boolean isGetParentField, boolean isGetAllParentField, boolean isDESCGet){
        ...
        if(isGetParentField){
            ...
            if(isGetAllParentField){
                classList = getSuperClass(sourceClass, true);
            }else{
     ...
     * @return 给定类的所有方法
     */
    public static List<Method> getMethods(Class<?> clazz, boolean isGetDeclaredMethod, boolean isFromSuperClassGet, boolean isDESCGet){
        List<Method> methodList = new ArrayList<Method>();
        //如果需要从父类中获取
        if(isFromSuperClassGet){
            //获取当前类的所有父类
            List<Class<?>> classList = getSuperClass(clazz, true);
            ...
            for(Method method : isGetDeclaredMethod ? clazz.getDeclaredMethods() : clazz.getMethods()){
                methodList.add(method);
            }
        }
        return methodList;
    ...
    public static List<Constructor<?>> getConstructors(Class<?> sourceClass, boolean isFindDeclaredConstructor, boolean isFromSuperClassGet, boolean isDESCGet){
        ...
        if(isFromSuperClassGet){
            //获取当前类的所有父类
            List<Class<?>> classList = getSuperClass(sourceClass, true);
     ...
     * @return 给定的类所有的父类
     */
    public static List<Class<?>> getSuperClass(Class<?> sourceClass, boolean isAddCurrentClass){
        List<Class<?>> classList = new ArrayList<Class<?>>();
        Class<?> clazz;
        if(isAddCurrentClass){
            clazz = sourceClass;
        }else{
            clazz = sourceClass.getSuperclass();
        }
        while(clazz != null){
            classList.add(clazz);
            clazz = clazz.getSuperclass();
    ...
    public static <T> T getObjectByFieldName(Object object, String fieldName, Class<T> clazz){
        if(object != null && !TextUtils.isEmpty(fieldName) && clazz != null){
            try {
